...
class TestEBS(testtools.TestCase):
    ...
    @mock_ec2
    def test_bad_volume_delete(self):
        """ tests that delete raises an error when
        the volume does not exist in the users account.
        """

        ctx = self.mock_ctx('test_bad_volume_delete')
        current_ctx.set(ctx=ctx)
        ctx.instance.runtime_properties['aws_resource_id'] = \
            BAD_VOLUME_ID
        ex = self.assertRaises(
            NonRecoverableError, ebs.delete, ctx=ctx)
        self.assertIn(
            'because the volume id does not exist in the account',
            ex.message)
