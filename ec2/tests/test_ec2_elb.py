...
from ec2 import elasticloadbalancer
...
class TestLoadBalancer(testtools.TestCase):
    ...
    def test_add_instance_to_elb(self):
        ...
        current_ctx.set(ctx=ctx)
        elasticloadbalancer.add_instance_to_elb(ctx=ctx)
        self.assertEqual(1,
    ...
    def test_delete_external_elb(self):
        ...
        self.assertFalse(elasticloadbalancer.delete_elb(ctx=elb_ctx))
    ...
    def test_get_elbs(self):
        ...
        self.assertRaises(NonRecoverableError,
                          elasticloadbalancer._get_elbs_by_names,
                          ['fake'])
    ...
    def test_get_instance_list(self):
        ...
        self._create_external_elb()
        current_ctx.set(ctx=ctx)
        l = elasticloadbalancer._get_instance_list()
        self.assertEquals([], l)
    ...
    @mock_elb
    def test_client_error_get_elbs_by_names(self):
        with mock.patch('ec2.elasticloadbalancer._get_elbs_by_names',
            ...
            self.assertRaises(boto.exception.BotoClientError,
                              elasticloadbalancer._get_instance_list)

    @mock_elb
    def test_client_error_create_elb(self):
        with mock.patch('ec2.connection.ELBConnectionClient',
            ...
            current_ctx.set(ctx=ctx)
            self.assertRaises(boto.exception.BotoClientError,
                              elasticloadbalancer.create_elb,
                              ctx=ctx)

    @mock_elb
    def test_client_error_add_health_check_to_elb(self):
        with mock.patch('ec2.connection.ELBConnectionClient',
                        new=self.mock_elb_client_raise_NonRecoverableError):
            ctx = self.mock_elb_ctx('test_client_error_get_elbs_by_names',
                                    use_external_resource=False,
            ...
            current_ctx.set(ctx=ctx)
            self.assertRaises(NonRecoverableError,
                              elasticloadbalancer.add_instance_to_elb,
                              ctx=ctx)
